<template>
  <v-layout v-if="showLayers" class="fill-height layers d-flex flex-column" style="background-color: bisque;">
    ...
    <div class="tools d-flex align-center">
      <v-btn x-small class="ml-2" color="primary" @click="addLayer">Create layer</v-btn>
      <v-spacer></v-spacer>
      <v-tooltip bottom>
    ...
    <v-layout class="list-layers align-content-start flex-wrap">
      <draggable v-model="layers" :options="dragOptions" @update="handleDragUpdate">
        <transition-group>
          <v-layout
          ...
          >
            ...
            <div class="control">
              <v-btn v-if="layer.view" icon x-small class="btn-control" @click="removeLayerFromMap(layer, i)">
                <v-icon small>mdi-eye</v-icon>
              </v-btn>
              <!-- <v-tooltip bottom v-else>
                <template v-slot:activator="{ attrs, on }"> -->
                  <v-btn v-else icon x-small class="btn-control" @click="addLayerToMap(layer, i)">
